...
CORS(APP, resources={r"/api/*": {"origins": "http://localhost:3000"}}) # Apenas o endereço de produção
...
# Define a global variable to store the token and its expiry time
token_cache = {
    'access_token': None,
    'expiry_time': None
}
...
@APP.route("/api/validtoken", methods=["POST"])
@require_auth()
...
@require_auth()
def get_token():
    # Check if the token is in the cache and hasn't expired
    print("cache")
    print(token_cache['access_token'])
    print(token_cache['expiry_time'])
    
    if token_cache['access_token'] and datetime.now() < token_cache['expiry_time']:
        print("Returning cached token")
        return jsonify({
            "access_token": token_cache['access_token'],
            "expiry_time": token_cache['expiry_time'].isoformat()
        })
    print("Returning new token")
    # If the token isn't in the cache or has expired, request a new one
